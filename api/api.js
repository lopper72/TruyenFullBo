...
// @param:
//  -url: Đường link chapter truyện. Ví dụ: https://truyenfull.vision/tiem-tap-hoa-cua-nhoc-con/chuong-1/
...
// @param:
//  - url: đường link của truyện. Ví dụ: https://truyenfull.vision/tiem-tap-hoa-cua-nhoc-con/
// @return: trả về một Json
// {
//    name: 'Tên truyện',
//    imgStory: '',
...
export const getAllChapters = (truyenID, urlStory) =>
  new Promise((resolve, reject) => {
    let url = `https://truyenfull.vision/ajax.php?type=chapter_option&data=${truyenID}`;
    fetch(url)
      .then(res => res.text())
      .then(html => cheerio.load(html))
      .then($ => {
...
export const getStoriesBySearch = keyword =>
  new Promise((resolve, reject) => {
    let url = `https://truyenfull.vision/tim-kiem/?tukhoa=${keyword}`;
    fetch(url)
      .then(res => res.text())
      .then(html => cheerio.load(html))
      .then($ => {
